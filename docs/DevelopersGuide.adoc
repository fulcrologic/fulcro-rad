...
"Elapsed time: 3.105000 msecs"
=> 499500
(time (reduce math/+ 0 (range 0 1000)))
"Elapsed time: 216.255000 msecs"
=> #bigdec"499500"
-----
...
NOTE: `with-primitive-ops` does not affect JVM performance or accuracy since the behavior of `with-primitive-ops` is to
call Clojure's pre-defined `+`, etc. In CLJ, these will continue to provide accuracy at a higher runtime cost.  This
primarily exists for cases where you're doing something in a UI that must render quickly, but that uses data in
this numeric format. For example a dynamically-adjusting report where you know the standard math to be accurate
enough for transient purposes.
