...
(defattr name :account/email :string
  {ao/required? true
   ao/identities #{:account/id}})
...
nature of such a thing is simply quite annoying (order-dependent, you can forgot requires, etc.).
...
bottom of each model namespace, then you can easily a list of all attributes like this:
...
few standard ones any you've created elsewhere. The result will look something like this:
...
The EQL network API of RAD is supplied by [Pathom Resolvers] that can pull the data
