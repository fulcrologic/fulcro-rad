...
it returns true then that item should show a delete button (depends on UI implementation).
* `::form/can-add-row?` - A lambda that receives the current parent (account). If
it returns true then the UI (if able) will include some kind of add control for adding a new row.
