...
You can also install middleware that can pre-process an incoming save so that it is fully compatible with the
built-in save that comes with your database adapter. Fulcro RAD will always send a form save in the following
format: (see Fulcro Form State docs)
...
Your middleware is a `(fn [env save-data] save-data)` that will receive the full server parsing `env` along with
the data that is to be saved. Your middleware should return the updated `save-data`.

You should install your middleware in the parser env under the `::form/middleware` key. If you are
using the RAD pathom parser, then this will look something like:

[source]
-----
(defn my-save-middleware [env params] ...)

(defstate parser
  :start
  (pathom/new-parser config
    (fn [env] (assoc env ::form/save-middleware my-save-middleware))
    [...
     form/save-form
     form/delete-entity
     ...]))
...
This could be used for pretty much anything, but common examples include:

* Adding default required fields that should not appear on the form (i.e. ownership info).
* Aborting the save (by throwing an exception)
* Creating or updating derived fields (e.g. Datomic tuple values) from other form data.
...
NOTE: Not yet implemented. Fulcro has all of the necessary network-layer implementation, so it should not be much of
a stretch to get this working.

RAD Forms can support file uploads, along with download/preview of previously-uploaded files. This support requires
the configuration of a file store that can be used to save the file itself, along with proper configuration to
associate a URL with the file's content so that the result can be seen later by RAD.
