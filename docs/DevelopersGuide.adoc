...
* Hard to figure out which `defmethod` is being called for a particular bit of output.

However, after experiencing the map-based mechanism, it suffers from similar issues:

* Still hard to trace the rendering to figure out what is generating output.
* Dead code elimination tuning is probably not going to save you much, given that these renderers are few in number
* It's actually kind of hard to plug in anything except a new field/formatter. Revamping some subset of the UI isn't very
fun.
...
The result was generally positive: It is much easier to use the multimethods to customize how things render, and the
...
bw compatibility with the older way of building a rendering plugin.
The other methods are defined to establish possible useful patterns.

NOTE: If you enable dev-time source line attribution in the compiler options you can at least see where a DOM element came from
in the source. See the https://book.fulcrologic.com/#_shadow_cljs_build_tool_configuration[Fulcro Developer's Guide].

===== Dispatch for `render-form`, `render-fields`, `render-header` and `render-footer`.

The dispatch uses a vector of `[qualified-keyword style]`, where the `style` is derived as follows (the first
matching rule wins) (for style option: headers use `fro/header-style`, footers use `fro/footer-style`, and fields use `fro/style`).:

* Subform options on parent form: If the form is being used as a subform AND the subform options for that
  reference (which can be declared on a reference attribute via `fo/subform` or parent form via `fo/subforms`)
  specifies the style option.
* Form: If the form *itself* has the style option.
* Attribute: If the attribute has the style option.
* Otherwise `:default` is used (resulting in [keyword :default]).
...
===== Dispatch for `render-field`

Field dispatch returns a `[data-type style]`. The data type is take from the attribute via `ao/type`. The style is:

* The value for the qualified-keyword in the form's `fo/field-styles`
* The value of `fo/field-style` on the attribute
* The value of `ao/style` on the attribute
