...
A set of attribute keys that are `:db/unique :db.unique/identity`. This
set indicates that the attribute in question can be placed on an entity that has one of those identity attributes.
...
TODO: Write the generator, and document it.
...
should be used in the context of the current request.
