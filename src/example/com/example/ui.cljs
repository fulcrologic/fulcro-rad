...
(defsc AccountList [this {:keys [::acct/all-accounts] :as props}]
  {
   ;; body item is NOT optional, but could be a one-liner, or
   ;; perhaps a list of attrs that the macro can use to auto-gen it
   ::report/BodyItem         AccountListItem

   ::report/source-attribute ::acct/all-accounts
   ::report/parameters       {:ui/active? :boolean}
   :route-segment            ["accounts"]

   ;; Macro generates the rest:
   ::rad/type                ::rad/report
   ::rad/io?                 true                           ; could be either...if the report doesn't auto-run it should be false? Possible to do it in the report SM as well
    ...
    (div :.ui.top.menu
      ...
      (dom/a :.ui.item {:onClick (fn []
                                   (controller/route-to! this :main-controller
                                     ["account" "edit" (str (new-uuid 1))]))} "My Account")
      (dom/a :.ui.item {:onClick (fn []
                                   (controller/route-to! this :main-controller
                                     ["account" "create" (str (new-uuid))]))} "New Account")
      (dom/a :.ui.item {:onClick (fn []
                                   (controller/route-to! this :main-controller ["accounts" "all"]))} "List Accounts"))
